/*
1. 爬取视频信息
	* 视频的基础信息  title/ bv号/  描述/
2. 将信息写入postgresql数据库中
3. 目标： 爬取能被查出的全部视频数据，
4.
*/
...
func main() {
	...
	var getVideoInfo = func(ctx *goribot.Context) {
		...
		fmt.Println(ctx.Resp.IsJSON(), "----------- CTX request")
		res := ctx.Resp.Body
	...
	findVideo = func(ctx *goribot.Context) {
		...
		ctx.Resp.Dom.Find("a[href]").Each(func(i int, sel *goquery.Selection) {
			if h, ok := sel.Attr("href"); ok {
				...
				if len(req1.Host) > 0 {
					// fmt.Println("Host --> ", req1.Host)
				} else {
					// fmt.Println("Not-Host-->", h)
				}
				ctx.AddTask(req1, findVideo)
			}
		})
	}
	s.OnItem(func(i interface{}) interface{} {
		...
		// fmt.Println(res["bvid"], "------------------bvid")
		// return i
		if len(i.([]byte)) < 2 {
			fmt.Println("test... OnItem  跳过...x")
			return i
		}
		var b Bilibili
		fmt.Println(string(i.([]byte)))
		err := json.Unmarshal(i.([]byte), &b)
		if err != nil {
			panic(err)
		}
